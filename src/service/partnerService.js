...
const registerClinic = async (Clinic) => {
  try {
    ...
    let findUser = await db.Users.findOne({
      where: { email: Clinic.emailUserOfClinicRegister },
    });
...
const registerDoctorClinic = async (dataDoctors) => {
  return new Promise(async (resolve, reject) => {
    try {
      let emailexist = [];
      await Promise.all(
        Object.entries(dataDoctors).map(async ([key, child], index) => {
          let findEmail = await db.Users.findOne({
            where: { email: child.doctorEmail },
          });
          ...
          if (findEmail && findEmail.dataValues) {
            ...
            let createNewUser = await db.Users.create({
              //đăng ký doctor
      ...
      include: [
        {
          model: db.Users,
          attributes: {
            exclude: ["password", "googleId", "githubId", "facebookId"],
          },
          include: [
            {
...
const updateDoctorOfClinic = async (dataDoctors) => {
  return new Promise(async (resolve, reject) => {
    try {
      console.log("check data Doctor >>", dataDoctors);
      let findUser = await db.Users.findOne({
        where: { id: dataDoctors.idUser },
      });
      let finDoctorInfo = await db.Doctorinfo.findOne({
        where: { id: dataDoctors.idDoctorInfo },
      });

      if (findUser) {
        let findPhoneExist = await db.Users.findOne({
          where: {
