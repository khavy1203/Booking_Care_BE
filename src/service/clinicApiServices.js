...
const fetchDoctorOfCLinic = async (page, limit, id) => {
  try {
    ...
    //lấy danh sách các chuyên khoa có trong phòng khám đó
...
const searchClinic = async (search, page, limit) => {
  try {
    console.log("check page and limit >>>", page, limit, search);
    if (page && limit && search != "") {
      let isActive = -1;
      if ("đang hoạt động".includes(search.toLowerCase())) isActive = 1;
      if ("tạm dừng".includes(search.toLowerCase())) isActive = 2;
      if ("đợi duyệt".includes(search.toLowerCase())) isActive = 0;
      let offset = (page - 1) * limit;
      const { count, rows } = await db.Clinics.findAndCountAll({
        offset: offset,
        limit: limit,
