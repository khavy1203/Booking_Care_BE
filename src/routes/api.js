...
const initApiRoutes = (app) => {
  ...
  routes.get("/auth/google/callback", passport.authenticate("google", {}), loginRegisterController.handleLoginPPGoogleSuccess);//callback dữ liệu
  // routes.get("/google/loginsuccess", loginRegisterController.handleLoginPPGoogleSuccess);
  //github
  routes.get("/github", passport.authenticate("github", { scope: ["profile", "email"] }));
  routes.get("/auth/github/callback", passport.authenticate("github"), loginRegisterController.handleLoginPPGithubSuccess);//callback dữ liệu
